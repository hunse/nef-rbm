...
class RBM(object):
    ...
    def sampHgivenV(self, vis):
        ...
        hidsamp = tt.maximum(hidact + eta, 0)
    ...
    def get_cost_updates(self, data, rate=0.1, weightcost=2e-4, momentum=0.5):
        ...
        # params = [self.W, self.b, self.c]
        # incs = [Winc, binc, cinc]
        # sincs = [self.Winc, self.binc, self.cinc]

        # c = 0.001
        # Wnorm = tt.sum(tt.abs_(Winc))
        # Winc = tt.switch(Wnorm > c, (c / Wnorm) * Winc, Winc)
        # for i, inc in enumerate(incs):
        #     inorm = tt.sum(tt.abs_(inc))
        #     incs[i] = tt.switch(inorm > c, (c / inorm) * inc, inc)

        # updates = [(p, p + inc) for p, inc in zip(params, incs)]
        # updates += [(sinc, inc) for sinc, inc in zip(sincs, incs)]
